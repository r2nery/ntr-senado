...
gen_url = lambda page, page_size, data_inicio, data_fim: f"https://www.camara.leg.br/internet/sitaqweb/resultadoPesquisaDiscursos.asp?CurrentPage={page}&BasePesq=plenario&dtInicio={data_inicio}&dtFim={data_fim}&CampoOrdenacao=dtSessao&TipoOrdenacao=ASC&PageSize={page_size}"
    ...
    with alive_bar(num_citations) as bar:
        for page in range(1, num_pages):
            ...
            for row in rows:
                ...
                df = pd.DataFrame([cols_data], columns=["Data", "Sessão", "Fase", "URL_Discurso", "Discurso", "Orador", "Hora", "Publicação"])
